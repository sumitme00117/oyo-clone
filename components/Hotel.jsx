...
const Hotel = ({e}) => {
  return (
    <div className="border-2 border-red-500 rounded-lg h-65 w-full mb-5 p-5">
      <div className="flex">
        <Image src={e?.banner} alt="hotel" width={200} height={200} className="w-96 h-58 mr-3"/>
        <div className="flex flex-col justify-between">
          {
            e ? e.gallery?.map((ele) =>{
              return (
                <Image key={ele} src={ele} alt="hotel" width={200} height={200} className="w-30 h-14"/>
              )
            }) : ""
          }
        ...
        <div className="ml-20">
            ...
            <Link href={`/hotels/${e?._id}`} className="text-xl font-bold text-green-600 ml-10">See Details</Link>
            </div>
        </div>
      </div>
    </div>
